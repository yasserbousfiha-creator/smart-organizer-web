...
import 'english_quiz_storage.dart';
...
class _EnglishQuizScreenState extends State<EnglishQuizScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final day = await EnglishQuizStorage.fetchToday();
      if (!mounted) return;
      setState(() {
        _day = day;
        _loading = false;
  ...
  Future<void> _answer(int questionId, int chosenIndex) async {
    ...
    try {
      final updated = await EnglishQuizStorage.submitAnswer(questionId, chosenIndex);
      if (!mounted) return;
      setState(() {
        _day = updated;
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text('أسئلة انجليزية'),
        ),
        body: _loading
  ...
  Widget _buildBody() {
    final day = _day!;
    final questions = day.questionIds.map((id) => kEnglishQuizBank.firstWhere((q) => q.id == id)).toList();
    return ListView(
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              const Expanded(
                child: Text(
                  '5 أسئلة يوميا، كل جواب صحيح = نقطتين تنضافوا لتحدي الصلاة',
                  style: TextStyle(color: Colors.white70, fontSize: 12.5),
                ),
              ),
              const SizedBox(width: 12),
              Text(
                '${day.points} / $kEnglishQuizMaxPoints',
  ...
  Widget _buildQuestionTile(int number, EnglishQuizQuestion question, EnglishQuizDay day) {
